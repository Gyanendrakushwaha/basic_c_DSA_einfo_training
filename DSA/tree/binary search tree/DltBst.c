...
struct node *delete(struct node *root,int data){
	//check if root is NULL or not if null means no data to delete ,return
if(root==NULL){
return root;
}
else{
	//if data to be deleted is less than the current root->data, means data would be in left
	//move left
	if(data<root->data){
	root->left=delete(root->left,data);
	}
	//if data to be deleted is greater than the current root->data, means data would be in right
	//move right
	else if(data>root->data){
	...
	//case 1
	//deleting data if reached at left  leaf node null 
	else{
		if(root->left
	}
}
